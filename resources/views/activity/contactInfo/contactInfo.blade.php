                ...
                <div class="activities__content">
                    <div class="py-[6.06%] px-[12%] bg-white">
                        ...
                        <div class="hidden collection-container title" form_type="title"
                            data-prototype="{{ form_row($form->organisation->getChildren()[0]->getChild('narrative')->prototype()) }}">
                        </div>
                        <div class="hidden collection-container description" form_type="description"
                            data-prototype="{{ form_row($form->department->getChildren()[0]->getChild('narrative')->prototype()) }}">
                        </div>
                        <div class="hidden collection-container description" form_type="description"
                            data-prototype="{{ form_row($form->person_name->getChildren()[0]->getChild('narrative')->prototype()) }}">
                        </div>
                        <div class="hidden collection-container description" form_type="description"
                            data-prototype="{{ form_row($form->job_title->getChildren()[0]->getChild('narrative')->prototype()) }}">
                        </div>
                        <div class="hidden collection-container telephone" form_type="telephone"
                            data-prototype="{{ form_row($form->telephone->prototype()) }}">
                        </div>
                        <div class="hidden collection-container email" form_type="email"
                            data-prototype="{{ form_row($form->email->prototype()) }}">
                        </div>
                        <div class="hidden collection-container website" form_type="website"
                        data-prototype="{{ form_row($form->website->prototype()) }}">
                        </div>
                        <div class="hidden collection-container description" form_type="description"
                            data-prototype="{{ form_row($form->mailing_address->getChildren()[0]->getChild('narrative')->prototype()) }}">
                        </div>
                    </div>
                </div>
            </div>
