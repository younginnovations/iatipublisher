    ...
    <?php if ($showLabel && $options['label'] !== false && $options['label_show']) : ?>
        <div class="form-field-label items-center">
            <?php if ($showLabel && $options['label'] !== false && $options['label_show'] && $options['required']) : ?>
                <?= Html::decode(Form::label($name, trans($options['label']) . '<span class="required-icon"> *</span>')) ?>
            <?php endif; ?>

            <?php if ($showLabel && $options['label'] !== false && $options['label_show'] && !$options['required']) : ?>
                <?= Html::decode(Form::label($name, trans($options['label']))) ?>
            <?php endif; ?>

            <?php if (isset($options['hover_block']) && $options['hover_block'] !== false) : ?>
                <div class="help"><svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 14" class="text-sm">
                    ...
                    <div class="help__text right-0 w-72">
                        <p class="text-bluecoral mb-2 italic"><?php echo translateCommon('iati_standard_reference') ?></p>
                        <span class="font-bold text-bluecoral"> <?= trans(lcfirst($options['hover_block']['title'])) ?></span>
                        <p><?= trans($options['hover_block']['text']) ?></p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
