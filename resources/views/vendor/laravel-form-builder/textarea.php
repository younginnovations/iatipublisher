...
if ($showLabel && $showField) : ?>
    ...
    <div class="form-field-label items-center">
        <?php if ($showLabel && $options['label'] !== false && $options['label_show'] && $options['required']) : ?>
            <?= Html::decode(Form::customLabel($name, trans($options['label']) . '<span class="required-icon"> *</span>')) ?>
        <?php endif; ?>

        <?php if ($showLabel && $options['label'] !== false && $options['label_show'] && !$options['required']) : ?>
            <?= Html::decode(Form::customLabel($name, trans($options['label']))) ?>
        <?php endif; ?>
        <?php if ($options['hover_block'] !== false) : ?>
            <div class="help"><svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 14" class="text-sm">
                ...
                <div class="help__text right-0 w-72">
                    ...
                    <span class="font-bold text-bluecoral"> <?= trans($options['hover_block']['title']) ?></span>
    ...
    <?php if ($showField) : ?>
        ...
        <?php if ($options['help_block']['text']) : ?>
            <div class="flex">
                <p class="help-button text-xs text-n-40 hover:text-spring-50 mb-1 cursor-pointer inline-block"><?= translateCommon('help') ?></p>
                <div class="help-button-content hidden">
                    <p class="font-bold text-bluecoral">
                        <?= trans($options['help_block']['title']) ?>
                    </p>
