<?php if ($showLabel && $showField) : ?>
    <?php if (isset($options['options']['dynamic_wrapper'])) : ?>
        ...
        <?php if ($showLabel && $options['label'] !== false && $options['label_show'] && strtolower($options['label']) !== "narrative") : ?>
            <?php if ($options['options']['element_criteria'] === 'mandatory') : ?>
                <?= HTML::decode(Form::customLabel($name, '<svg-vue icon="star" class="mr-2"></svg-vue>' . $options['label'], $options['label_attr'])) ?>
            <?php elseif ($options['options']['element_criteria'] === 'recommended') : ?>
                <?= HTML::decode(Form::customLabel($name, '<svg-vue icon="moon" class="mr-2"></svg-vue>' . $options['label'], $options['label_attr'])) ?>
            <?php else : ?>
