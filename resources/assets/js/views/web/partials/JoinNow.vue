  ...
  >
    <div class="right__container flex flex-col">
      <h2 class="mb-2 hidden sm:block">
        {{ translatedData['login.iati_publishing_tool_section.join_now'] }}
      </h2>
      <span class="mb-8 text-n-40 xl:pr-6">
        {{ translatedData['login.join_now_section.to_begin_text'] }}
      </span>
      <a href="/iati/register" class="right__content mb-6">
        ...
        <div class="details mx-4 xl:px-1">
          <span class="text-sm font-bold text-bluecoral">{{
            translatedData['login.join_now_section.new_to_iati_label']
          }}</span>
          <p
            class="text-xs leading-5 text-n-40"
            v-html="translatedData['login.join_now_section.new_to_iati_text']"
          ></p>
        </div>
        <div>
      ...
      <a href="/register" class="right__content mb-4">
        ...
        <div class="details mx-4 xl:px-1">
          <span class="text-sm font-bold text-bluecoral">{{
            translatedData[
              'login.join_now_section.my_organisation_has_registered_label'
            ]
          }}</span>
          <p
            class="text-xs leading-5 text-n-40"
            v-html="
              translatedData[
                'login.join_now_section.my_organisation_has_registered_text'
              ]
            "
          ></p>
        </div>
      ...
      <span class="text-sm text-n-40">
        {{ translatedData['login.join_now_section.information_text'] }}
        <a
          ...
          target="_blank"
          >{{
            translatedData['login.join_now_section.contact_support_label']
          }}</a
        ></span
      >
    </div>
