<template>
  ...
  <div class="list-disc rounded-md bg-salmon-10 p-3 font-medium fw-bold">
    ...
    <ul class="list-disc rounded-md bg-salmon-10 p-3 font-medium">
      <li
        ...
        :key="index"
        class="py-2 pr-2 flex justify-between"
      >
        <span>{{ activity.activity_title }}</span>
        <span :class="getActivityClass(activity.status)">{{ activity.status }}</span>
      </li>
    </ul>
    Please wait for previous bulk publish to complete or cancel previous bulk publish to continue this bulk publish.
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      axios.get("activities/organisation-bulk-publish-status")
        .then((res) => {
