<?php

return [
    'transactions_fetched_successfully'                                => 'Transactions fetched successfully',
    'error_has_occurred_while_rendering_activity_transaction_form'     => 'Error has occurred while rendering activity transaction form.',
    'activity_transaction_created_successfully'                        => 'Activity transaction created successfully.',
    'error_has_occurred_while_creating_activity_transaction'           => 'Error has occurred while creating activity transaction.',
    'error_has_occurred_while_rendering_transaction_detail_page'       => 'Error has occurred while rendering transaction detail page.',
    'error_has_occurred_while_updating_activity_transaction'           => 'Error has occurred while updating activity transaction.',
    'activity_transaction_updated_successfully'                        => 'Activity transaction updated successfully.',
    'transaction_deleted_successfully'                                 => 'Transaction Deleted Successfully',
    'transaction_delete_error'                                         => 'Transaction Delete Error',
    'transactions_deleted_successfully'                                => 'Transactions Deleted Successfully',
    'failed_to_bulk_delete_transactions'                               => 'Failed to bulk delete transactions.'
];
