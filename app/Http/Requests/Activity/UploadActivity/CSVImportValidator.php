        ...
        foreach ($transactions as $transactionIndex => $transactionRow) {
            $rules = array_merge(
                ...
                [
                    ...
                    "$transactionIndex.transactiontype_code"        => 'in:' . $transactionTypeCodes,
                    "$transactionIndex.transactiondate_iso_date"    => 'date',
                    "$transactionIndex.transactionvalue_value_date" => 'date',
                    "$transactionIndex.transactionvalue_text"       => 'numeric',
                    "$transactionIndex.description_text"            => '',
            ...
            foreach ($activities as $activityIndex => $activityRow) {
                $rules = array_merge(
                    ...
                    [
                        ...
                        // ),
                        "$activityIndex.activity_identifier"                => sprintf(
                            'sometimes|unique_validation:%s.activity_identifier,%s,%s,activity_identifier|not_in:%s',
                            $activityIndex,
                            trimInput($activityRow['activity_identifier']),
                            $file,
                            $identifiers
                        ...
                        // ),
                        "$activityIndex.actual_start_date"                  => 'date',
                        ...
                        "$activityIndex.activity_status"                    => 'in:' . $activityStatus,
                        "$activityIndex.sector_dac_5digit"                  => 'multiple_value_in:' . $sectorCategory,
                        "$activityIndex.recipient_country"                  => 'sometimes|multiple_value_in:' . $recipientCountryCodes,
                        "$activityIndex.recipient_region"                   => 'sometimes|multiple_value_in:' . $recipientRegionCodes,
                    ]
                );
        ...
        foreach ($transactions as $transactionIndex => $transactionRow) {
            ...
            $rules = array_merge(
                ...
                [
                    ...
                    "$transactionIndex.transaction_date"     => 'sometimes|date',
