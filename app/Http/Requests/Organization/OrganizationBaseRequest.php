...
class OrganizationBaseRequest extends FormRequest
{
            ...
            function ($attribute, $value, $parameters, $validator) {
                ...
                $validator->addReplacer(
                    ...
                    function ($message, $attribute, $rule, $parameters) use ($validator, $defaultLanguage) {
                        return str_replace(':language', app('App\Helpers\GetCodeName')->getActivityCodeName('Language', $defaultLanguage), $message);
                    }
                );
    ...
    public function getMessagesForNarrative($formFields, $formBase)
    {
        ...
        foreach ($formFields as $narrativeIndex => $narrative) {
            $messages[sprintf('%s.narrative.%s.narrative.required', $formBase, $narrativeIndex)] = trans('validation.required', ['attribute' => 'Title']);
            $messages[sprintf(
    ...
    public function getRulesForPeriodEnd($formFields, $formBase)
    {
        ...
        foreach ($formFields as $periodEndKey => $periodEndVal) {
            ...
            $rules[$formBase . '.period_end.' . $periodEndKey . '.date'] = sprintf('nullable|date|after:%s', $formBase . '.period_start.' . $periodEndKey . '.date');
        }
