...
class StatusController extends Controller
{
    ...
    public function edit(int $id): View|RedirectResponse|JsonResponse
    {
        try {
            ...
            $translatedData = trans('activity_detail/status_controller.error_has_occurred_while_opening_activity_title_form');
    ...
    public function update(StatusRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            ...
            if (!$this->statusService->update($id, $activityStatus)) {
                $translatedData = trans('activity_detail/status_controller.error_has_occurred_while_updating_activity_status');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/status_controller.activity_status_updated_successfully');
