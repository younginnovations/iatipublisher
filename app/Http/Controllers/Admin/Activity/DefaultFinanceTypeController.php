...
class DefaultFinanceTypeController extends Controller
{
    ...
    public function edit(int $id): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('activity_detail/default_finance_type_controller.error_has_occurred_while_rendering_default_finance_type_form');
    ...
    public function update(DefaultFinanceTypeRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            ...
            if (!$this->defaultFinanceTypeService->update($id, ['code' => $activityDefaultFinanceType])) {
                $translatedData = trans('activity_detail/default_finance_type_controller.error_has_occurred_while_updating_default_finance_type');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/default_finance_type_controller.default_finance_type_updated_successfully');
