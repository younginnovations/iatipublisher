...
class LocationController extends Controller
{
    ...
    public function edit(int $id): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('activity_detail/location_controller.error_has_occurred_while_rendering_location_form');
    ...
    public function update(LocationRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            if (!$this->locationService->update($id, $request->except(['_token', '_method']))) {
                $translatedData = trans('activity_detail/location_controller.error_has_occurred_while_updating_location');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/location_controller.location_updated_successfully');
