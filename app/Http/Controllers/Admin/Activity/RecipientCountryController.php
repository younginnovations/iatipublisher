...
class RecipientCountryController extends Controller
{
    ...
    public function edit(int $id): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('activity_detail/recipient_country_controller.error_has_occurred_while_opening_recipient_country_form');
    ...
    public function update(RecipientCountryRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            if (!$this->recipientCountryService->update($id, $request->all())) {
                $translatedData = trans('activity_detail/recipient_country_controller.error_has_occurred_while_updating_recipient_country');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/recipient_country_controller.recipient_country_updated_successfully');
    ...
    public function getRecipientCountryManipulatedElementSchema($activity): array
    {
        ...
        if (count($activity->transactions)) {
            ...
            if (!is_array_value_empty($recipient_region) || !is_array_value_empty($recipient_country)) {
                $element['freeze'] = true;
                $translatedData = trans('activity_detail/recipient_country_controller.recipient_country_is_already_added_at_transaction_level');

                $element['info_text'] = $translatedData;
            }
        }
