...
class TitleController extends Controller
{
    ...
    public function edit(int $id): Factory|View|RedirectResponse|Application
    {
        try {
            ...
            $translatedData = trans('activity_detail/title_controller.error_has_occurred_while_opening_activity_title_form');
    ...
    public function update(TitleRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            if (!$this->titleService->update($id, $request->all())) {
                $translatedData = trans('activity_detail/title_controller.error_has_occurred_while_updating_activity_title');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/title_controller.activity_title_updated_successfully');
