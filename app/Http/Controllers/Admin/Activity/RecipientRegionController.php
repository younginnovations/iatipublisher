...
class RecipientRegionController extends Controller
{
    ...
    public function edit(int $id): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('activity_detail/recipient_region_controller.error_has_occurred_while_opening_recipient_region_form');
    ...
    public function update(RecipientRegionRequest $request, $id): JsonResponse|RedirectResponse
    {
        try {
            if (!$this->recipientRegionService->update($id, $request->all())) {
                $translatedData = trans('activity_detail/recipient_region_controller.error_has_occurred_while_updating_recipient_region');

                return redirect()->route('admin.activity.show', $id)->with('error', $translatedData);
            }
            $translatedData = trans('activity_detail/recipient_region_controller.recipient_region_updated_successfully');
