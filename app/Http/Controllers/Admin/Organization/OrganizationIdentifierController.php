...
class OrganizationIdentifierController extends Controller
{
    ...
    public function edit(): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('organisationDetail/organisation_identifier_controller.error_has_occurred_while_opening_organization_identifier_form');
...
//            }
            ...
            if ($this->organizationIdentifierService->update($id, $organizationIdentifier)) {
                DB::commit();
                $translatedData = trans('organisationDetail/organisation_identifier_controller.organisation_identifier_updated_successfully');

                return redirect()->route('admin.organisation.index')->with('success', $translatedData);
            }

            DB::rollBack();
            $translatedData = trans('organisationDetail/organisation_identifier_controller.error_has_occurred_while_updating_organisation_identifier');
