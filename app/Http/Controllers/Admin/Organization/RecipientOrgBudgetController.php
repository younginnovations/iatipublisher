    ...
    public function edit(): View|RedirectResponse
    {
        try {
            ...
            return redirect()->route('admin.organisation.index')
                ->with('error', translateErrorHasOccurred('responses.org_recipient_org_budget', 'opening', 'form'));
        }
    }
    ...
    public function update(RecipientOrgBudgetRequest $request): RedirectResponse
    {
        try {
            if (!$this->recipientOrgBudgetService->update(Auth::user()->organization_id, $request->all())) {
                return redirect()->route('admin.organisation.index')
                    ->with('error', translateErrorHasOccurred('responses.org_recipient_org_budget', 'updating'));
            }

            return redirect()->route('admin.organisation.index')
                ->with('success', translateElementSuccessfully('responses.org_recipient_org_budget', 'updated'));
        } catch (\Exception $e) {
            logger()->error($e->getMessage());
