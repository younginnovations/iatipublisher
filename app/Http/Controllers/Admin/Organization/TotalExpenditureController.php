...
class TotalExpenditureController extends Controller
{
    ...
    public function edit(): View|RedirectResponse
    {
        try {
            ...
            $translatedData = trans('organisationDetail/total_expenditure_controller.error_has_occurred_while_opening_organization_total_expenditure_form');
    ...
    public function update(TotalExpenditureRequest $request): RedirectResponse
    {
        try {
            if (!$this->totalExpenditureService->update(Auth::user()->organization_id, $request->all())) {
                $translatedData = trans('organisationDetail/total_expenditure_controller.error_has_occurred_while_updating_organization_total_expenditure');

                return redirect()->route('admin.organisation.index')->with('error', $translatedData);
            }
            $translatedData = trans('organisationDetail/total_expenditure_controller.organization_total_expenditure_updated_successfully');
