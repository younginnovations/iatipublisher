...
class OrganizationOnboardingController extends Controller
{
    ...
    public function toggleDontShow(Request $request): JsonResponse
    {
        try {
            ...
            DB::commit();
            $translatedData = trans('organisationDetail/organisation_onboarding_controller.dont_show_again_updated_successfully');

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (\Exception $e) {
            DB::rollBack();
            logger()->error($e->getMessage());
            $translatedData = trans('organisationDetail/organisation_onboarding_controller.error_occurred_while_updating_dont_show_again');
    ...
    public function completeActivityForOnboarding(): JsonResponse
    {
        try {
            ...
            DB::commit();
            $translatedData = trans('organisationDetail/organisation_onboarding_controller.activity_status_updated_successfully');

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (\Exception $e) {
            DB::rollBack();
            logger()->error($e->getMessage());
            $translatedData = trans('organisationDetail/organisation_onboarding_controller.error_occurred_while_storing_updating_activity_status');
