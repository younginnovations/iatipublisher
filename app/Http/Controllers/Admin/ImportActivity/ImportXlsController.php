...
class ImportXlsController extends Controller
{
    ...
    public function index(): View|JsonResponse|RedirectResponse
    {
        try {
            if (!Auth::user()->organization_id) {
                $translatedData = trans('import_activity/import_xls_controller.user_is_not_associated_with_any_organization');
            ...
            $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_rendering_activity_import_page');
    ...
    public function store(ImportXlsRequest $request): JsonResponse
    {
        try {
            ...
            if (ImportCacheHelper::hasOngoingImport($orgId) || !empty($status) || Arr::get($ongoingImportStatus, 'status') === 'processing') {
                $translatedData = trans('import_activity/import_xls_controller.import_is_currently_on_progress_please_cancel_the_current_import_to_continue');
            ...
            $translatedData = trans('import_activity/import_xls_controller.uploaded_successfully');
    ...
    public function importValidatedActivities(Request $request): mixed
    {
        try {
            ...
            if (empty($status)) {
                $translatedData = trans('import_activity/import_xls_controller.please_ensure_that_you_have_uploaded_xls_file');

                return response()->json(['success' => false, 'message' => $translatedData]);
            }

            if (empty($activities)) {
                $translatedData = trans('import_activity/import_xls_controller.please_select_the_data_you_want_to_add');
            ...
            if ($xlsType === 'activity' && !ImportCacheHelper::organisationHasCompletedValidatingData(Auth::user()->organization_id)) {
                $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_importing_the_data');

                return response()->json(['success' => false, 'message' =>  $translatedData, 'type' => $xlsType]);
            }
            ...
            ImportCacheHelper::clearImportCache(Auth::user()->organization_id);
            $translatedData = trans('import_activity/import_xls_controller.xls_file_with_xlstype_imported_successfully');

            Session::put('success', $translatedData);

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (Exception $e) {
            $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_importing_activity');
            Session::put('error', $translatedData);
    ...
    public function checkImportInProgressForPolling(): JsonResponse
    {
        try {
            ...
            $translatedData = trans('import_activity/import_xls_controller.import_status_accessed_successfully');

            return response()->json(['success' => true, 'message' => $translatedData, 'status' => $status]);
        } catch (Exception $e) {
            logger()->error($e->getMessage());
            $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_trying_to_check_import_status');
    ...
    public function checkStatus(): JsonResponse
    {
        try {
            ...
            if (empty($status)) {
                $translatedData = trans('import_activity/import_xls_controller.please_upload_xls_file_to_import_activity');
    ...
    public function deleteImportError($activityId): JsonResponse
    {
        try {
            ...
            $translatedData = trans('import_activity/import_xls_controller.import_error_for_activity_has_been_successfully_deleted');

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (Exception $e) {
            logger()->error($e->getMessage());
            $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_trying_to_delete_import_error');
    ...
    public function deleteImportStatus(): JsonResponse
    {
        try {
            ...
            DB::commit();
            $translatedData = trans('import_activity/import_xls_controller.import_status_for_organization_has_been_successfully_deleted');
    ...
    public function show(): View|JsonResponse|RedirectResponse
    {
        try {
            ...
            if (empty($status)) {
                $translatedData = trans('import_activity/import_xls_controller.please_upload_xls_file_to_continue');
            ...
            logger()->error($e->getMessage());
            $translatedData = trans('import_activity/import_xls_controller.error_has_occurred_while_opening_import_listing_page');
