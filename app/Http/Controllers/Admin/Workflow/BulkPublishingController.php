...
class BulkPublishingController extends Controller
{
    ...
    public function checkCoreElementsCompleted(Request $request): JsonResponse
    {
        try {
            ...
            if ($this->publishingStatusService->ongoingBulkPublishing(auth()->user()->organization->id)) {
                ...
                $translatedData = trans('work_flow/bulk_publishing_controller.another_bulk_publishing_is_already_in_progress');
            ...
            if (!empty($activityIds)) {
                ...
                DB::commit();
                $translatedData = trans('work_flow/bulk_publishing_controller.retrieved_data_successfully');
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.no_activities_selected');
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_checking_core_elements_completed');

            return response()->json(
                ['success' => false, 'message' => $translatedData]
            );
        }
    }
        ...
        try {
            ...
            if (!empty($activityIds)) {
                if ($hasUploadedCache || count($this->bulkPublishingService->checkOngoingValidationStatus())) {
                    $translatedData = trans('work_flow/bulk_publishing_controller.another_activity_validation_already_in_progress');
            ...
            logger()->error($e->getMessage());
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_validating_activities');
    ...
    public function startBulkPublish(Request $request): JsonResponse
    {
        try {
            ...
            if (!empty($activityIds)) {
                ...
                DB::commit();
                $translatedData = trans('work_flow/bulk_publishing_controller.bulk_publishing_started');

                return response()->json(
                    ['success' => true, 'message' => $translatedData, 'data' => $response]
                );
            }
            $translatedData = trans('work_flow/bulk_publishing_controller.no_activities_selected');
            ...
            logger()->error($e->getMessage());
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.bulk_publishing_failed');
    ...
    public function getBulkPublishStatus(): JsonResponse
    {
        try {
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.no_bulk_publishing_in_progress');

            return response()->json(
                ['success' => false, 'message' => $translatedData, 'publishing' => false]
            );
        } catch (Exception $e) {
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.status_generation_failed');

            return response()->json(['success' => false, 'message' => $translatedData]);
        } catch (NotFoundExceptionInterface|ContainerExceptionInterface $e) {
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.request_error');
    ...
    public function cancelBulkPublishing(): JsonResponse
    {
        try {
            ...
            if ($deletedIds) {
                DB::commit();
                $translatedData = trans('work_flow/bulk_publishing_controller.bulk_publish_of_numberofdeletedrows_activities_canceled');
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.no_bulk_publish_were_cancelled');
            ...
            logger()->error($e->getMessage());
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.failed_to_stop_bulk_publishing');
    ...
    public function checksForActivityBulkPublish(): JsonResponse
    {
        try {
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.activity_is_ready_to_be_published');

            return response()->json(
                ['success' => true, 'message' => $translatedData, 'status' => 'completed']
            );
        } catch (Exception $e) {
            logger()->error($e->getMessage());
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_checking_activity');
    ...
    public function checksForActivityBulkValidation(): JsonResponse
    {
        try {
            ...
            if ($hasUploadedCache || count($ongoingValidationActivities)) {
                $translatedData = trans('work_flow/bulk_publishing_controller.another_bulk_validation_is_already_in_progress');
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.activities_are_ready_to_validate');
    ...
    public function clearBulkPublishStatus(): JsonResponse
    {
        try {
            ...
            DB::commit();
            $translatedData = trans('work_flow/bulk_publishing_controller.bulk_publishing_status_successfully_deleted');
            ...
            logger()->error($e);
            DB::rollBack();
            $translatedData = trans('work_flow/bulk_publishing_controller.failed_to_delete_bulk_publishing_status');
    ...
    public function getValidationStatus(Request $request): JsonResponse
    {
        try {
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.activity_not_selected');

            return response()->json(['success' => false, 'message' => $translatedData]);
        } catch (Exception|\Throwable $e) {
            logger()->error($e);
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_fetching_validation_status');
    ...
    public function getValidationResponse(Request $request): JsonResponse
    {
        try {
            ...
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_fetching_validation_response');
    ...
    public function deleteValidationStatus(): JsonResponse
    {
        try {
            ...
            Cache::forget('activity-validation-' . auth()->user()->id);
            $translatedData = trans('work_flow/bulk_publishing_controller.successfully_deleted');

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (Exception|\Throwable $exception) {
            logger()->error($exception);
            $translatedData = trans('work_flow/bulk_publishing_controller.error_has_occurred_while_deleting_validation_status');
