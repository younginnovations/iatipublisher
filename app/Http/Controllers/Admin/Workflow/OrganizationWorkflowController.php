...
class OrganizationWorkflowController extends Controller
{
    ...
    public function publish(): JsonResponse
    {
        try {
            ...
            DB::commit();
            $translatedData = trans('work_flow/organization_workflow_controller.organization_has_been_published_successfully');
            ...
            $translatedData = trans('work_flow/organization_workflow_controller.error_has_occurred_while_publishing_organization');
    ...
    public function unPublish(): JsonResponse|RedirectResponse
    {
        try {
            ...
            if (!$organization->is_published && $organization->status === 'draft') {
                $translatedData = trans('work_flow/organization_workflow_controller.this_organization_has_not_been_published_to_un_publish');
            ...
            DB::commit();
            $translatedData = trans('work_flow/organization_workflow_controller.organization_has_been_un_published_successfully');

            return response()->json(['success' => true, 'message' => $translatedData]);
        } catch (\Exception $e) {
            DB::rollBack();
            logger()->error($e->getMessage());
            $translatedData = trans('work_flow/organization_workflow_controller.error_has_occurred_while_un_publishing_organization');
    ...
    public function checksForOrganizationPublish(): JsonResponse
    {
        ...
        $translatedData = trans('work_flow/organization_workflow_controller.organization_is_ready_to_be_published');
