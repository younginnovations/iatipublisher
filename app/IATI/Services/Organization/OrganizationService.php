...
class OrganizationService
{
     ...
     */
    public function updateBySuperadmin(array $organizationData): void
    {
        $organization = $this->organizationRepo->find($organizationData['organization_id']);
        $oldOrgInstance = clone $organization;
        $dbField = $organization->getFillable();
        $organizationData = Arr::only($organizationData, $dbField);
        ...
        if ($publisherIdHasChanged) {
            $this->beginPublisherIdChangeWorkflow($organizationData, $oldOrgInstance, $organization);
        }
     ...
     * @throws JsonException
     */
    private function beginPublisherIdChangeWorkflow(array $orgData, Organization $oldOrgInstance, Organization $org): void
    {
        $oldPublisherId = $oldOrgInstance->publisher_id;
        $settings = $org->settings;
        $publisherId = $orgData['publisher_id'];
        $orgApiToken = Arr::get($settings->publishing_info, 'api_token', '');
        $publisherData = [
