...
class ParentCollectionForm extends BaseForm
{
    ...
    {
        return 'multi-form relative one';
    }
    ...
    private function getBaseFormDynamicWrapperClasses($field): string
    {
        ...
        if ($hasAddMoreButton) {
            if ($isSubElementNarrative && $elementHasAttributes) {
                ...
                $dynamicWrapperClass = "subelement rounded-t-sm six border-spring-50  $formBorderClass $labelWithBorder";
            }
        } else {
            $dynamicWrapperClass = "subelement rounded-t-sm seven border-spring-50 pb-4 $formBorderClass $labelWithBorder";
        }
