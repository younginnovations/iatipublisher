...
class OrganizationXmlGenerator
{
    ...
    public function getMergeXml($publishedFiles, $filename)
    {
        ...
        $xml = $publishedFiles->filename;

        $addDom = new \DOMDocument();
        $fileContent = Storage::disk('minio')->get(sprintf('%s/%s/%s', 'xml', 'organizationXmlFile', $xml));

        if ($fileContent) {
            Storage::disk('local')->put('public/xml/organizationXmlFiles/' . $xml, $fileContent);
        }

        $file = Storage::disk('local')->path('public/xml/organizationXmlFiles/' . $xml);
        $addDom->load($file);
        ...
        if (Storage::disk('local')->exists('public/xml/organizationXmlFiles/' . $xml)) {
            Storage::disk('local')->delete('public/xml/organizationXmlFiles/' . $xml);
        }
