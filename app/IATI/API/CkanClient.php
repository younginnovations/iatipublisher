...
class CkanClient
{
    ...
    private function set_headers()
    {
        ...
        if (env('APP_ENV') != 'production') {
            $this->curl_headers[] = 'X-CKAN-API-Key: ' . env('IATI_API_KEY');
            $this->curl_headers[] = 'Authorization: Basic ' . base64_encode(env('IATI_USERNAME') . ':' . env('IATI_PASSWORD'));
        }
    }
    ...
    private function make_request($method, $uri, $data = null)
    {
        ...
        // app()->make(IatiApiLogRepository::class)->store(generateApiInfo($method, $uri, is_array($data) ? $data : [], $response));

        try {
            $responseArray = json_decode($response, true);

            if ($responseArray['success'] == false && array_key_exists('error', $responseArray)) {
                $responseErrors = $responseArray['error'];

                if (is_array($responseErrors)) {
                    if (array_key_exists('__type', $responseErrors)) {
                        unset($responseErrors['__type']);
                    }

                    session()->put('registry_errors.list', []);

                    foreach ($responseErrors as $errorList) {
                        if (is_array($errorList)) {
                            foreach ($errorList as $error) {
                                session()->push('registry_errors.list', $error);
                            }
                        } else {
                            session()->push('registry_errors.list', $errorList);
                        }
                    }
                } else {
